...
def Kmeans(data,k,eps,init_num = 10):
    ...
    return centroids_list[loss_list.argmin()],assign_points(data,centroids_list[loss_list.argmin()]),[Image.open(f) for f in fig_list[loss_list.argmin()]]
...
def hierarchical_clustering(X,method = upgma, distance_measurement = 'euclidean'):
    ...
    return Clusters_list,steps,[Image.open(f) for f in fig_list]
...
def DBSCAN(X, eps, MinPts):
    ...
    return labels,[Image.open(f) for f in fig_list]
